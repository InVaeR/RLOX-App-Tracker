...
def test_needs_migration_with_old_data(tmp_path: Path, monkeypatch):
    ...
    (old_dir / "tracker.db").write_text("sqlite data")
...
def test_migration_skipped_if_marker_exists(tmp_path: Path, monkeypatch):
    mig_dir = tmp_path / "migration"
    mig_dir.mkdir(parents=True)
    (mig_dir / "rusloxpy-1.x.json").write_text("{}")
